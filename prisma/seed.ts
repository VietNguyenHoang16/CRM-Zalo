import { PrismaClient, TemplateGroup } from '@prisma/client'
...
async function main() {
  ...
  await Promise.all([
    ...
    prisma.messageTemplate.create({
      data: {
        title: 'Chăm sóc sau mua hàng',
        content: 'Chào {ten},\n\nCảm ơn anh/chị đã tin tưởng lựa chọn sản phẩm bên em. Em chỉ muốn hỏi thăm anh/chị dùng sản phẩm có tốt không ạ? Nếu cần hỗ trợ gì cứ nhắn em nhé.',
        group: 'CHAM_SOC' as TemplateGroup,
      },
    }),
    prisma.messageTemplate.create({
      data: {
        title: 'Chúc mừng sinh nhật',
        content: 'Chúc mừng sinh nhật {ten}! 🎂\n\nChúc anh/chị một ngày sinh nhật thật vui vẻ và hạnh phúc bên gia đình. Bên em có món quà nhỏ dành tặng anh/chị, ghé qua nhận nhé!\n\n{nguon} - {sdt}',
        group: 'CHAM_SOC' as TemplateGroup,
      },
    }),
    prisma.messageTemplate.create({
      data: {
        ...
        content: 'Xin chào {ten},\n\nDạo này anh/chị và gia đình vẫn khỏe chứ ạ? Bên em đang có chương trình kiểm tra sức khỏe miễn phí cho khách hàng từ {nguon}. Anh/chị quan tâm em gửi thông tin chi tiết nhé.',
        group: 'CHAM_SOC' as TemplateGroup,
      },
    }),
    prisma.messageTemplate.create({
      data: {
        title: 'Nhắc lịch hẹn gặp',
  ...
  await Promise.all([
    prisma.customer.create({
      data: {
        ...
        sourceId: sourceFacebook.id,
        status: 'PENDING_ZALO' as CustomerStatus,
      },
    }),
    prisma.customer.create({
      data: {
        name: 'Trần Thị Bình',
        phone: '0909123457',
        sourceId: sourceZalo.id,
        status: 'ZALO_CONNECTED' as CustomerStatus,
    ...
    prisma.customer.create({
      data: {
        ...
        phone: '0909123460',
        sourceId: sourceZalo.id,
        status: 'MESSAGED' as CustomerStatus,
      },
    }),
  ])
  ...
  await prisma.settings.upsert({
    ...
    create: {
      id: 'global',
      tgChoAloSauKB: 60,
      tgChoNhantinSauGoiKhongNghe: 30,
      tgLapLichNhantinLan2: 2,
    },
  })
