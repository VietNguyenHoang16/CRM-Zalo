...
import { useDashboard, useUrgentCustomers } from '@/hooks/queries'
...
export default function DashboardPage() {
  ...
  const { data: urgentCustomers, isLoading: urgentLoading } = useUrgentCustomers()

  const isLoading = statsLoading || urgentLoading

  const emptyStats = { todayCustomers: 0, pendingZalo: 0, pendingCall: 0, contactedCount: 0, messagedCount: 0, total: 0, conversionRate: 0 }
  const sdata = stats ?? emptyStats
  const urgents = urgentCustomers ?? []
    ...
    <div>
      ...
      <div className="grid-4" style={{ marginBottom: 24 }}>
        <div className="stat-card blue">
          ...
          <div className="stat-value">{isLoading ? '...' : sdata.todayCustomers}</div>
        ...
        <div className="stat-card yellow">
          ...
          <div className="stat-value">{isLoading ? '...' : sdata.pendingZalo}</div>
        ...
        <div className="stat-card orange">
          ...
          <div className="stat-value">{isLoading ? '...' : sdata.pendingCall}</div>
        ...
        <div className="stat-card green">
          ...
          <div className="stat-value">{isLoading ? '...' : sdata.contactedCount}</div>
      ...
      <div className="grid-2">
        <div className="card">
          ...
          {urgentLoading ? (
            <p style={s.emptyText}>Đang tải...</p>
          ) : urgents.length === 0 ? (
        ...
        <div className="card">
          ...
          <div style={s.overviewGrid}>
            <div style={s.statRow}><span style={s.statLabel}>Tổng khách hàng</span><span style={s.statValue}>{isLoading ? '...' : sdata.total}</span></div>
            <div style={s.statRow}><span style={s.statLabel}>Tỷ lệ chuyển đổi</span><span style={s.statValueGreen}>{isLoading ? '...' : `${sdata.conversionRate}%`}</span></div>
            <div style={s.statRow}><span style={s.statLabel}>Đã nhắn tin</span><span style={s.statValuePurple}>{isLoading ? '...' : sdata.messagedCount}</span></div>
            <div style={s.statRow}><span style={s.statLabel}>Khách hàng hôm nay</span><span style={s.statValueBlue}>{isLoading ? '...' : sdata.todayCustomers}</span></div>
          </div>
        </div>
      </div>
    </div>
